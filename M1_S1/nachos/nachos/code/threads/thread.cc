...
void
Thread::Init(const char *threadName) {
    name = threadName;
    stackTop = NULL;
    stack = NULL;
    status = JUST_CREATED;
...
#ifdef STEP3
Thread::Thread (const char *threadName, UserThread* userThread_arg)
{
    Init(threadName);
    userThread = userThread_arg;
}
#endif
...
#ifdef STEP3
UserThread* Thread::getUserThread() {
    return userThread;
}
#endif
