...
int run_login_command(command_t *cmd)
{
    ...
    client_data->commands_received = 0;
    client_data->commands_finished = 0;
    pthread_mutex_init(&client_data->commands_finished_mutex, NULL);
    pthread_cond_init(&client_data->commands_finished_cond, NULL);
    ...
    pthread_rwlock_init(&client_data->pub_lock, NULL);
    pthread_rwlock_init(&client_data->followed_lock, NULL);
...
{    
    ...
    pthread_rwlock_wrlock(&client->pub_lock);
    publication_t *pub = publication_set_insert(client->pub_set, cmd->msg);    
    pthread_rwlock_unlock(&client->pub_lock);
...
int run_follow_command(command_t *cmd)
{
    ...
    pthread_rwlock_wrlock(&client->followed_lock);
    ...
    if (i == client->nb_followed) {
        ...
        fetch_and_add(&f_client->nb_follower, 1);
    }		
    pthread_rwlock_unlock(&client->followed_lock);
...
int run_timeline_command(command_t *cmd)
{
    ...
    /* gather publications over all followed clients */
 		pthread_rwlock_rdlock(&client->followed_lock);
    for(i=0; i < client->nb_followed; i++){    		   		
        client_data_t *f_client=client->followed[i];
        pthread_rwlock_rdlock(&f_client->pub_lock);
        ...
        pthread_rwlock_unlock(&f_client->pub_lock);
    }
 		pthread_rwlock_unlock(&client->followed_lock);
...
void ensure_order(client_data_t *client, command_t *cmd) {
    pthread_mutex_lock(&client->commands_finished_mutex);
    while (client->commands_finished < cmd->order) {
        pthread_cond_wait(&client->commands_finished_cond, &client->commands_finished_mutex);
    }
...
int run_unregister_command(command_t *cmd)
{
    ...
    if (client == NULL) {
        fprintf(stderr,"Warning -- failed to unregister a client (key %lu) [%lu]\n", cmd->key, cmd->order);
        return -1;
    }
    ...
    /* remove client */
    registration_remove(cmd->key);
    printf("### Unregister client %s (key = %lu) [%lu][%lu]\n", client->client_name, client->key, cmd->order, client->commands_finished);
