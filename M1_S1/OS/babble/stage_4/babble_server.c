...
{
	while(1) {
        command_t* cmd = buffer_get_command();
		exec_command(cmd);
	}
...
{	
    ...
    while (1) {
        bool is_client;
		void* item = buffer_get_client_or_command(&is_client);
        if (!is_client) {
            exec_command((command_t*) item);
            continue;
        }


        int newsockfd = *((int*) item);
        free(item);
...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        *sockfd_thread = newsockfd;
        buffer_add_client(sockfd_thread);
    }
