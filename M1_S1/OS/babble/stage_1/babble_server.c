...
void* comm_thread(void* sockfd_thread)
{	
	int newsockfd = *((int*) sockfd_thread);
    free(sockfd_thread);
...
int main(int argc, char *argv[])
{
    ...
    pthread_t executor;
    pthread_create(&executor, NULL, exec_thread, NULL);
    ...
    while (1) {
        ...
        *sockfd_thread = newsockfd;
    	pthread_create(&comm, NULL, comm_thread, sockfd_thread);
    }
